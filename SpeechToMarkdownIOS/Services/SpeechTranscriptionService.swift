...
final class SpeechTranscriptionService {
    ...
    func ensureAssets(preferring preferred: Locale = .current) async throws {
        ...
        stmdLog("STT", "Preferred locale: \(preferred.identifier(.bcp47)); supported: \(supported.map { $0.identifier(.bcp47) }.joined(separator: ", "))")
        ...
        if locale.identifier(.bcp47) != preferred.identifier(.bcp47) {
            stmdLog("STT", "Locale \(preferred.identifier) unsupported; falling back to \(locale.identifier(.bcp47))")
        }
    ...
    func finalizeNow() async {
        ...
        do {
            ...
            stmdLog("STT", "finalize(through:) failed: \(error.localizedDescription)")
        }
    }
