...
final class STMDSessionTests: XCTestCase {
    ...
    private func makeSession(model: ModelSize = .base, format: OutputFormat = .md) -> STMDSession {
        STMDSession(modelSize: model, baseDir: baseDir, format: format)
    }
    ...
    func testDirPathContainsSpeechToMarkdown() {
        let session = makeSession()
        XCTAssertTrue(session.dirPath.path.contains("speech-to-markdown"))
    }
    ...
    private func makeListing(id: String = "1000", format: OutputFormat = .md) -> SessionListing {
        SessionListing(
            id: id,
            dirPath: baseDir.appendingPathComponent("speech-to-markdown/\(id)"),
            format: format,
    ...
    func testRestoringPreservesIdDirPathAndFormat() {
        ...
        let session = STMDSession(restoring: listing, modelSize: .small)
    ...
    func testRestoringUsesProvidedModelSize() {
        let session = STMDSession(restoring: makeListing(), modelSize: .large)
        XCTAssertEqual(session.modelSize, .large)
    }

    func testRestoringStateIsIdle() {
        let session = STMDSession(restoring: makeListing(), modelSize: .base)
        XCTAssertEqual(session.state, .idle)
    }

    func testRestoringDocPathMatchesListingFormat() {
        let listing = makeListing(format: .html)
        let session = STMDSession(restoring: listing, modelSize: .base)
