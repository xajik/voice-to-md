...
final class STMDLogger {
    static let shared = STMDLogger()

    private let queue = DispatchQueue(label: "com.stmd.logger", qos: .utility)
    ...
    func configure(logsDir: URL) {
        queue.async {
            ...
            let name = "stmd_\(nameFormatter.string(from: Date())).log"
...
func stmdLog(_ category: String, _ message: String) {
    STMDLogger.shared.log(category, message)
}
