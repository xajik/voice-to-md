...
final class SessionCoordinator: ObservableObject {
    ...
    func startSession(modelSize: ModelSize) async {
        ...
        stmdLog("SESSION", "Starting session: model=\(modelSize.rawValue)")
        transcript = ""
        markdown = ""
        error = nil
        mode = .format
        ...
        var newSession = STMDSession(modelSize: modelSize, baseDir: fileManager.stmdRoot, format: outputFormat)
        ...
        do {
            ...
            stmdLog("SESSION", "Ready: model=\(llmModel) doc=\(newSession.docPath.path)")
    ...
    /// Loads a prior session from disk (see `STMDFileManager.listSessions()`)
    ...
    func restoreSession(_ listing: SessionListing) async {
        ...
        var restored = STMDSession(restoring: listing, modelSize: modelSize)
        ...
        do {
            ...
            stmdLog("SESSION", "Error restoring session: \(error.localizedDescription)")
            self.error = error.localizedDescription
            llmService = nil
            session = nil
        }
    ...
    private func transcribe(buffers: [AVAudioPCMBuffer]) async {
        ...
        let wavURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("stmd_chunk_\(UUID().uuidString).wav")
        ...
        do {
            ...
            stmdLog("WHISPER", "Transcribed: \(text)")
            transcript += text + " "
            ...
            stmdLog("WHISPER", "Transcription error: \(error.localizedDescription)")
        }
    }
}
